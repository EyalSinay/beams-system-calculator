...
const DistributedLoads = ({ X0, Y0, name, position1, position2, loadValue1, loadValue2, loadValue1Relative, loadValue2Relative }) => {
    ...
    return (
        <g className="PointLoad">
            ...
            <text fontSize={F_S} x={X0 + position1 + (F_S / 2) * loadDirection1} y={Y0 + 0.5 * loadValue1Relative} transform={`rotate(${90 * loadDirection1},${X0 + position1 + (F_S / 2) * loadDirection1},${Y0 + 0.5 * loadValue1Relative})`} >{`${name} = ${loadValue1.toFixed(2)}`}</text>

            <text fontSize={F_S} x={X0 + position2 - (F_S) * loadDirection2} y={Y0 + 0.5 * loadValue2Relative} transform={`rotate(${90 * loadDirection2},${X0 + position2 - (F_S) * loadDirection2},${Y0 + 0.5 * loadValue2Relative})`} >{`${name} = ${loadValue2.toFixed(2)}`}</text>
        </g>
    );
