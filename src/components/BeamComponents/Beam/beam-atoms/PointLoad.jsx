...
const PointLoad = ({ X0, Y0, name, position, loadValue }) => {
    ...
    return (
        <g className="PointLoad">
            ...
            <text fontSize={F_S} x={X0 + position + (F_S / 2) * loadDirection} y={Y0 + 0.5 * loadValue} transform={`rotate(${90 * loadDirection},${X0 + position + (F_S / 2) * loadDirection},${Y0 + 0.5 * loadValue})`} >{`${name} = ${loadValue.toFixed(2)}`}</text>
        </g>
    );
